...
PROJECT_ID = "22"
ROOT_WIKI_SLUG = "Atomic-Relax-Artifacts"
ACCESS_TOKEN = os.environ.get('WIKI_ACCESS_TOKEN')
CSV_FILE_PATH = "/builds/endf/library/atomic_relax/.gitlab-ci/atomic_relax.csv"
...
def create_markdown_table(csv_file_path, headers = ["Atomic Number (Z)", "Symbol", "Atomic Mass (A)"]):
    ...
    # Remove "atom-" from string values
    data_no_n = np.core.defchararray.replace(csv_data, 'atom-', '')
    ...
    for row in numpy_array:
        ...
        if key in combined_values:
            combined_values[key][0] += f", [{value}]({ROOT_WIKI_SLUG}/atom-{key}_{symbol}_{value})"# Add the value with a comma
        else:
            combined_values[key] = [f"[{value}]({ROOT_WIKI_SLUG}/atom-{key}_{symbol}_{value})", symbol]  # Initialize with the value and value from column two
...
def update_all_jobs_to_wiki(job_name_to_find='verify_endf', per_page = 15):
    ...
    # Start the loop through the pages
