...
PROJECT_ID = "25"
ROOT_WIKI_SLUG = "Gammas-Artifacts"
ACCESS_TOKEN = os.environ.get('WIKI_ACCESS_TOKEN')
CSV_FILE_PATH = "/builds/endf/library/gammas/.gitlab-ci/gammas.csv"
...
def create_markdown_table(csv_file_path, headers = ["Atomic Number (Z)", "Symbol", "Atomic Mass (A)"]):
    ...
    data_no_n = np.core.defchararray.replace(csv_data, 'g-', '')
    ...
    for row in numpy_array:
        ...
        if key in combined_values:
            combined_values[key][0] += f", [{value}]({ROOT_WIKI_SLUG}/g-{key}_{symbol}_{value})"# Add the value with a comma
        else:
            combined_values[key] = [f"[{value}]({ROOT_WIKI_SLUG}/g-{key}_{symbol}_{value})", symbol]  # Initialize with the value and value from column two
