...
PROJECT_ID = "33"
ROOT_WIKI_SLUG = "Thermal-Neutron-Scattering"
ACCESS_TOKEN = os.environ.get('WIKI_ACCESS_TOKEN')
CSV_FILE_PATH = "/builds/endf/library/thermal_scatt/.gitlab-ci/thermal_scatt.csv"
...
def create_markdown_table(csv_file_path, headers = ["Material", "Files"]):
    ...
    # Remove "tsl-" from string values
    data_no_tsl = np.core.defchararray.replace(csv_data, 'tsl-', '')
    
    # Create a dictionary to store values and hyperlinks  as strings
    data_list = []
    material_dict = {}
    
    # Loop through the array
    for row in data_no_tsl:
        material = str(row[0])
        data_list.append(f"tsl-{material}")

        if 'in' in material:
            # Find the index of the substring
            index = material.find('in')
            # Get the value after the substring
            value = material[index + len('in'):]
            # Search dict to see if it already has an entry for that material
            if value in material_dict:
                current_data = material_dict.get(value)
                material_dict.update({f"{value}": (current_data + f", [{material}]({ROOT_WIKI_SLUG}/tsl-{material})")})
            else:
                #if not, add as a new row in the table
                material_dict.update({f"{value}": f"[{material}]({ROOT_WIKI_SLUG}/tsl-{material})"})
        else:
            # just a vanilla file
            material_dict.update({f"{material}": f"[{material}]({ROOT_WIKI_SLUG}/tsl-{material})"})

    # Convert the dictionary into an alphabetically sorted table
    # (using case insenstive sort while outputing original letter case)
    sorted_dict = sorted(material_dict.items(), key=lambda x: x[0].lower())
    sorted_dict_with_original_case = [(key, material_dict[key]) for key, _ in sorted_dict]
    combined_array = np.array(sorted_dict_with_original_case)
